...
async function findBestMatch(event: BetfastEvent, fixtures: CanonicalFixture[], bookmakerSlug: string) {
...
export function createBetfastCollector(bookmaker: BetfastBookmakerConfig) {
  return async function collectBetfast(options: BookmakerCollectOptions = {}) {
    ...
    try {
      ...
      const bestMatchByFixtureId = new Map<string, { event: BetfastEvent; matched: NonNullable<Awaited<ReturnType<typeof findBestMatch>>> }>();
