...
async function ensureBaseRows(bookmaker: AltenarBookmakerConfig) {
  const { error: bookmakerError } = await supabase.from("casas_apostas").upsert({ slug: bookmaker.slug, name: bookmaker.name }, { onConflict: "slug" });
...
async function getCanonicalFixtures() {
  ...
  const { data, error } = await supabase
    .from("jogos")
    .select("id,api_football_fixture_id,name,league:campeonatos(name,api_football_league_id),home_team,away_team,normalized_home_team,normalized_away_team,starts_at")
